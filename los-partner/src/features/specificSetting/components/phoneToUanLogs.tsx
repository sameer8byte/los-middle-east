...
import { PhoneToUanService, PhoneToUanLog, PhoneToUanStatus, PhoneToUanProvider } from "../services/phoneToUanService";

export function PhoneToUanLogsSetting() {
  const { brandId } = useParams<{ brandId: string }>();
  const [logs, setLogs] = useState<PhoneToUanLog[]>([]);
  const [loading, setLoading] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState<"all" | PhoneToUanStatus>("all");
  const [providerFilter, setProviderFilter] = useState<"all" | PhoneToUanProvider>("all");
  ...
  const fetchLogs = async () => {
    ...
    try {
      setLoading(true);
      const data = await PhoneToUanService.getFilteredLogs(
        brandId,
        page,
        limit,
      ...
      console.error("Error fetching Phone to UAN logs:", error);
      showError("Failed to load Phone to UAN logs. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredLogs = logs.filter(log => {
    ...
    return (
      log.mobileNumber?.toLowerCase().includes(searchLower) ||
  ...
  const downloadCSV = () => {
    const csvContent = [
      ['Date', 'Mobile Number', 'UAN', 'Provider', 'Status', 'User', 'Error'].join(','),
      ...filteredLogs.map(log => [
        formatDate(log.createdAt),
        maskMobileNumber(log.mobileNumber),
        log.uan ? maskUan(log.uan) : '',
        log.provider,
        log.status,
        log.user?.email || '',
    ...
    link.href = url;
    link.download = `phone-to-uan-logs-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
    window.URL.revokeObjectURL(url);
    
    showSuccess(`Successfully downloaded ${filteredLogs.length} Phone to UAN log records.`);
  };
  ...
  const maskMobileNumber = (mobile: string) => {
    ...
    return `${mobile.substring(0, 3)}XXXXX${mobile.substring(8)}`;
  };
    ...
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-[var(--color-on-background)]">
          Phone to UAN Verification Logs
        </h2>
        <p className="text-[var(--color-on-surface)] opacity-70 mt-2">
          View all phone to UAN verification attempts and results.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-4 items-center">
        <div className="flex-1 min-w-[250px]">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by mobile number, UAN, user email, or name..."
              value={searchTerm}
      ...
      {!loading && filteredLogs.length === 0 && (
        <div className="text-center py-8 text-[var(--color-on-surface)] opacity-70">
          ...
          <p>No Phone to UAN logs found</p>
        </div>
      )}
            ...
            <table className="w-full">
              <thead className="bg-[var(--color-muted)] bg-opacity-10">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-sm font-semibold text-[var(--color-on-background)]">Mobile Number</th>
            ...
            {filteredLogs
              ...
              .map((log) => (
                <div key={log.id} className="text-sm text-red-600 bg-red-50 p-2 rounded">
                  <span className="font-medium">{maskMobileNumber(log.mobileNumber)}</span>: {log.errorMessage}
                </div>
              ))}
          </div>
        </div>
