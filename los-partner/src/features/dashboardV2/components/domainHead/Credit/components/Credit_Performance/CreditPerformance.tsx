...
const StatBox = ({ label, value }: { label: React.ReactNode, value: React.ReactNode }) => (
    <div className="flex items-baseline gap-1 2xl:gap-2">
        <span className="text-gray-500 text-[11px] lg:text-[10px] 2xl:text-[13px] font-medium">{label}</span>
        <span className="text-gray-900 text-[12px] lg:text-[11px] 2xl:text-[14px] font-semibold">{value}</span>
    </div>
);

export default function CreditPerformance({ data = MOCK_DATA }: { data?: SalesPerformanceData }) {
    ...
    return (
        <div className="bg-[#fafafa] rounded-2xl overflow-hidden border border-gray-100 shadow-sm w-full font-sans">
            ...
            {/* --- Top Header --- */}
            <div
                className="px-4 py-3 2xl:px-6 2xl:py-4 flex justify-between items-center border-b border-gray-200/60 bg-white cursor-pointer select-none"
                onClick={() => setIsExpanded(!isExpanded)}
            >
                <h2 className="text-[14px] 2xl:text-[16px] font-semibold text-gray-800 tracking-tight">Credit Performance Analysis</h2>
            ...
            {isExpanded && (
                <div className="p-3 2xl:p-6 space-y-3 2xl:space-y-5">
                    ...
                    {/* --- Manager Banner Row --- */}
                    <div className="bg-white rounded-xl border border-gray-200 px-3 py-3 2xl:px-5 2xl:py-4 flex flex-wrap gap-2 2xl:gap-4 justify-between items-center">
                        <div className="flex flex-wrap items-center gap-3 2xl:gap-6">
                            <div className="text-[14px] lg:text-[13px] 2xl:text-[16px] font-bold text-gray-900 border-r border-gray-200 pr-3 2xl:pr-6">
                                {data.managerName}
                            </div>

                            <div className="flex items-center gap-1.5 2xl:gap-2">
                                <span className="text-gray-500 text-[12px] lg:text-[11px] 2xl:text-[14px] font-medium">Active Executives</span>
                                <span className="bg-[#3b82f6] text-white text-[11px] 2xl:text-[13px] font-bold px-1.5 py-0.5 2xl:px-2 rounded-md flex items-center justify-center">
                                    {data.activeExecutives}
                                </span>
                            </div>

                            <div className="flex items-center gap-1.5 2xl:gap-2">
                                <span className="text-gray-500 text-[12px] lg:text-[11px] 2xl:text-[14px] font-medium">Team Avg Rating</span>
                                <span className="bg-[#dcfce7] text-[#166534] text-[11px] 2xl:text-[13px] font-bold px-1.5 py-0.5 2xl:px-2 rounded-md flex items-center gap-1">
                                    {data.teamAvgRating} <StarIcon />
                                </span>
                            </div>

                            <div className="flex items-center gap-1.5 2xl:gap-2">
                                <span className="text-gray-500 text-[12px] lg:text-[11px] 2xl:text-[14px] font-medium">Manager Rating</span>
                                <span className="bg-[#dcfce7] text-[#166534] text-[11px] 2xl:text-[13px] font-bold px-1.5 py-0.5 2xl:px-2 rounded-md flex items-center gap-1">
                        ...
                        <button className="flex items-center gap-1.5 2xl:gap-2 px-3 py-1.5 2xl:px-4 2xl:py-2 border border-gray-200 rounded-lg text-[11px] lg:text-[10px] 2xl:text-sm font-semibold text-gray-700 hover:bg-gray-50 bg-white">
                            View Executive Performance
                            <div className="w-4 h-4 2xl:w-5 2xl:h-5 bg-[#3b82f6] rounded flex items-center justify-center shrink-0">
                                <CheckIcon />
                            </div>
                        </button>
                    </div>
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                        <div className="px-3 py-2.5 2xl:px-5 2xl:py-4 bg-[#f8fafc] border-b border-gray-200">
                            <h3 className="text-[12px] 2xl:text-[14px] font-bold text-gray-800">Executive Performance Summary</h3>
                        </div>

                        <div className="overflow-x-auto">
                            <table className="w-full text-left text-[11px] lg:text-[10px] 2xl:text-[13px]">
                                <thead className="bg-white border-b border-gray-200 text-gray-600 font-semibold sticky top-0">
                                    <tr>
                                        <th className="px-2 py-2.5 2xl:px-4 2xl:py-4 font-semibold whitespace-nowrap text-center">Sr No</th>
                                        <th className="px-2 py-2.5 2xl:px-4 2xl:py-4 font-semibold whitespace-nowrap">Executive</th>
                                        <th className="px-2 py-2.5 2xl:px-4 2xl:py-4 font-semibold whitespace-nowrap text-center">Leads</th>
                                        <th className="px-2 py-2.5 2xl:px-4 2xl:py-4 font-semibold whitespace-nowrap text-center">Followup</th>
                                        <th className="px-2 py-2.5 2xl:px-4 2xl:py-4 font-semibold whitespace-nowrap text-center">Disbursed</th>
                                        <th className="px-2 py-2.5 2xl:px-4 2xl:py-4 font-semibold whitespace-nowrap text-center">Rejected</th>
                                        <th className="px-2 py-2.5 2xl:px-4 2xl:py-4 font-semibold whitespace-nowrap text-center">Sanctioned Amount</th>
                                        <th className="px-2 py-2.5 2xl:px-4 2xl:py-4 font-semibold whitespace-nowrap text-center">Disbursed Amount</th>
                                        <th className="px-2 py-2.5 2xl:px-4 2xl:py-4 font-semibold whitespace-nowrap text-center">Pending Disbursal</th>
                                        <th className="px-2 py-2.5 2xl:px-4 2xl:py-4 font-semibold whitespace-nowrap text-center">Conversion %</th>
                                        <th className="px-2 py-2.5 2xl:px-4 2xl:py-4 font-semibold whitespace-nowrap text-center">Drop off rate</th>
                                        <th className="px-2 py-2.5 2xl:px-4 2xl:py-4 font-semibold whitespace-nowrap text-center">Performance Score</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100">
                                    {data.executives.map((exec, idx) => (
                                        <tr key={exec.id} className="hover:bg-gray-50/50 transition-colors">
                                            <td className="px-2 py-2 2xl:px-4 2xl:py-4 text-gray-500 font-medium text-center">{idx + 1}</td>
                                            <td className="px-2 py-2 2xl:px-4 2xl:py-4 text-gray-800 font-medium whitespace-nowrap">
                                                {exec.name} <span className="text-[#a855f7] font-bold ml-1">• {exec.role}</span>
                                            </td>
                                            <td className="px-2 py-2 2xl:px-4 2xl:py-4 text-gray-600 text-center">{exec.leads}</td>
                                            <td className="px-2 py-2 2xl:px-4 2xl:py-4 text-gray-600 text-center">{exec.followup}</td>
                                            <td className="px-2 py-2 2xl:px-4 2xl:py-4 text-gray-600 text-center">{exec.disbursed}</td>
                                            <td className="px-2 py-2 2xl:px-4 2xl:py-4 text-gray-600 text-center">{exec.rejected}</td>
                                            <td className="px-2 py-2 2xl:px-4 2xl:py-4 text-gray-800 font-medium text-center">{Conversion.formatCurrency(exec.sanctionedAmount)}</td>
                                            <td className="px-2 py-2 2xl:px-4 2xl:py-4 text-gray-800 font-medium text-center">{Conversion.formatCurrency(exec.disbursedAmount)}</td>
                                            <td className="px-2 py-2 2xl:px-4 2xl:py-4 text-gray-800 font-medium text-center">{Conversion.formatCurrency(exec.pendingDisbursalAmt)}</td>
                                            <td className="px-2 py-2 2xl:px-4 2xl:py-4 text-[#3b82f6] font-semibold text-center">{exec.conversionPercent}%</td>
                                            <td className="px-2 py-2 2xl:px-4 2xl:py-4 text-[#ef4444] font-semibold text-center">{exec.dropoffRatePercent}%</td>
                                            <td className="px-2 py-2 2xl:px-4 2xl:py-4 text-center">
                                                <div className="inline-flex items-center gap-1 bg-[#fef08a] text-[#854d0e] font-bold px-1.5 py-0.5 2xl:px-2 2xl:py-0.5 rounded-md text-[11px] 2xl:text-[13px]">
