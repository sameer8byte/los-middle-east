...
export default function LoanList() {
  ...
  const { searchTerm: selectedCollectionExecutives, setSearchTerm: setSelectedCollectionExecutives } = 
    usePersistedSearch<string[]>(
      "loanPostCollectionList_collectionExecutive",
  ...
  const { searchTerm: selectedCollectionSupervisors, setSearchTerm: setSelectedCollectionSupervisors } = 
    usePersistedSearch<string[]>("loanPostCollectionList_collectionSupervisor", []);
  ...
  const [pagination, setPagination] = useState<Pagination>(() => {
    const savedLimit = localStorage.getItem("loanPostCollectionListPageSize");
    const savedPage = localStorage.getItem("loanPostCollectionListPage");
    return {
...
---- LOAN COMPLETED ----
    ...
    () => [
                ...
                <div className="flex flex-col gap-1 text-xs">
                  <div className="flex items-center justify-between group/email">
                    <div className="flex items-center gap-1 flex-1 truncate max-w-[180px]">
                      ...
                      <span className="truncate">
                        {maskEmail(loan.user.email || "", isEmailVisible)}
                      </span>
                    </div>
                    {loan.user.email && (
                      <button
                  ...
                  <div className="flex items-center justify-between group/phone">
                    <div className="flex items-center gap-1 flex-1">
                      ...
                      <span className="truncate">
                        {maskPhone(loan.user.phoneNumber || "", isPhoneVisible)}
                      </span>
                    </div>
      ...
      // disbursed date
      {
        ...
        render: (_: any, loan: Loan) => {
          ...
          return (
            <div className="text-xs text-[var(--color-on-background)] flex flex-col gap-1">
              {/* Disbursed Date */}
              <div className="flex">
              ...
              <div className="flex">
                <span className="whitespace-nowrap font-medium">
                  Created – {formatDateWithTime(loan.createdAt)}
                </span>
              </div>
            </div>
          );
      ...
      },
      {
        ...
        render: (_: any, loan: Loan) => (
          <div className="flex flex-col items-center gap-2">
            <AddRemarksButton loanId={loan.id} onOpenModal={openCommentModal} />
            ...
            <Button
              onClick={(e) => {
                ...
                // handleView(loan.user.id, loan.id); --- IGNORE ---
              }}
              variant="outline"
            >
              View
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(() => {
    const saved = sessionStorage.getItem("postCollectionTabVisibleColumns");
    return saved ? JSON.parse(saved) : columns.map((col) => col.key);
  });

  useEffect(() => {
    sessionStorage.setItem(
      "postCollectionTabVisibleColumns",
