...
import { getRecording, getRepaymentTimeline } from "../../../../shared/services/api/collection.api";
  ...
  const handelRecording = async (userCallRecordingsId: string) => {
    ...
    try {
      ...
      setRepaymentTimeline((prev) =>
        prev.map((item) => {
          if (item.userCall?.recordings.some((r) => r.id === userCallRecordingsId)) {
            return {
              ...item,
              userCall: {
                ...item.userCall,
              ...
              {repaymentTimeline.length === 0 ? (
                ...
                <>
                  <ul className="space-y-6">
                    {repaymentTimeline && repaymentTimeline?.map((item) => (
                                      ...
                                      <div key={recording.id}>
                                        {recording.filePrivateUrl ? (
                                          ...
                                          <div>
                                            <button
                                              type="button"
                                              className={`flex items-center text-xs font-medium text-[var(--color-on-primary)] hover:text-[var(--color-on-primary)] transition ${recordingLoading
