...
export class CardService {
  ...
  private handleAxiosError(error: any, defaultMsg: string) {
    console.error("Detailed error in Cart Service:", {
    ...
    // Handle network errors like EPIPE
    if (error.code === 'EPIPE' || error.code === 'ECONNRESET') {
      throw new HttpException(
        {
          message: "Network connection error occurred while uploading to external service",
    ...
    try {
      ...
      if (data?.status === "Rejected") {
        throw new BadRequestException(
          data?.message || "Upload failed: Request rejected by service",
        );
      }
      ...
      throw new HttpException(
        data?.responseMessage || "Upload failed: Unexpected response status",
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    } catch (error) {
